...
void Rasteriser::shadowPass(int width, int height, vector<Pixel> &leftPixels,
	...
	for (int y = 0; y < static_cast<int>(leftPixels.size()); y++) {
		for (int x = l_pixels[y].x; x <= r_pixels[y].x; x++) {
			float pixelDepth = lerpF(l_pixels[y].depth, r_pixels[y].depth, deLerpF(l_pixels[y].x, r_pixels[y].x, x));
			Vertex pixelVert = lerpV(l_pixels[y].v, r_pixels[y].v, l_pixels[y].depth, r_pixels[y].depth, pixelDepth,
				deLerpF(l_pixels[y].x, r_pixels[y].x, x));
			float depth = 0;
