...
kernel void pathTrace(global const float3 *triangles, float3 lightLoc,
  ...
  image[(y * width + x)] =
      (float3){min(finalLight.x, 1.0f), min(finalLight.y, 1.0f),
...
kernel void standardShade(global const float3 *triangles, float3 lightLoc,
  ...
  image[(y * width + x)] =
      (float3){min(color.x, 1.0f), min(color.y, 1.0f), min(color.z, 1.0f)};
}
...
kernel void castRays(global const float3 *triangles, float3 lightLoc,
  ...
  float3 cameraPos =
      (float3){(float)camera[0], (float)camera[1], (float)camera[2]};
  Ray cameraRay;
  cameraRay.origin = cameraPos;

  float3 cameraSpaceDirection =
      (float3){lerpF(camera[3], -camera[3], (float)x / (float)width),
