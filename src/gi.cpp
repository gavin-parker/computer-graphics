...
vec3 GlobalIllumination::trace(Ray &ray, int bounces) {
  ...
  if (abs(normal.x) > abs(normal.y)) {
    normalX = vec3(normal.z, 0, -normal.x) /
              sqrtf(normal.x * normal.x + normal.z * normal.z);
  } else {
    normalX = vec3(0, -normal.z, normal.y) /
              sqrtf(normal.z * normal.z + normal.y * normal.y);
  }
  ...
  if (bounces >= 1) {
    ...
    for (int i = 0; i < sampleCount; i++) {
      ...
      if (boundingVolume->calculateIntersection(bounce)) {
        ...
        indirectLight +=
            r1 * vec3(1, 1, 1) * environment; // assume white environment sphere
      }
    }
  } else {
